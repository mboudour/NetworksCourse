...
def create_pandas_dataframe_from_paragraphs(paragraphs,selectedTerms,ndici,titlename):
    ...
    coccurlist =[]
    occurdict=Counter()
    all_sents=paragraphs#.sentences
...
def draw_network(G,sstt,pos={},with_edgewidth=False,withLabels=True,pernode_dict={},
    ...
    with_edgecolor=False,edgecolor='polarity',colormat='Blues',node_col=None):
    fig=plt.figure(figsize=(12,12))
    if len(pos)==0:
        pos=nx.spring_layout(G,scale=50)
    if node_col:
        nncol=[node_col[v] for  v in G.nodes()]
    else:
        nncol=['r' for nd in G.nodes()]
    if with_edgewidth:
        edgewidth=[]
    ...
    if with_node_weight:
        ...
        # print node_weights
        nx.draw_networkx_nodes(G,pos=pos,with_labels=False,alpha=valpha,node_size=node_weights,node_color=nncol)
    else:
        nx.draw_networkx_nodes(G,pos=pos,with_labels=False,alpha=valpha,node_color=nncol)
    if withLabels:
...
def community_layout(g, partition):
    ...
    pos_communities = _position_communities(g, partition, scale=2.) #3.
...
# def draw_circle_around_groups(groups,coords):
#     dist=0
#     temp_dist=0
