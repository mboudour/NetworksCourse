...
def influence_sim(nodes,p,sa,iterations,scale=1000):
    while  True:
    ...
    col=[F.nodes[i]['scalar_attribute'] for i in F.nodes()]
    ...
    iterat=[]
    assort=[]
    y=[F.nodes[i]['scalar_attribute'] for i in F.nodes()]
    ckck=4
    kckc=2
    for ii in range(iterations):
        ...
        # print nd
        uu=0
        nei=F.neighbors(nd) #nx.neighbors(F,nd)
        # print nei
        for nnei in nei:
...
def sidif_sim(nodes,p,b,iterations,scale=1000):
    ...
    # print col
    print [F.nodes[i]['scalar_attribute_numeric'] for i in F.nodes()]
    # pos=nx.spring_layout(G)
    ##nx.draw_networkx(g,pos=pos, node_color=col,cmap=plot.cm.Reds)
    nx.draw_networkx(G,pos=pos, node_color=col,vmin=0.,vmax=1.)
    plt.axis('equal')
